...
(function($) {
		...
		$(document.body).append(
			$([
				...
				bottom = $('<div id="pbBottom" />').append([
					...
					$('<div id="pbNav" />').append([
						leftBtn = $('<a id="pbLeftBtn" href="#" title="逆时针旋转" />').click(rotateLeft)[0],
						prevBtn = $('<a id="pbPrevBtn" href="#" title="上一张"  />').click(previous)[0],
						zoomBtn  = $('<a id="pbZoomBtn" href="#" title="原始尺寸"  />').click(doubleClick)[0],
						leftBtn = $('<a id="pbRightBtn" href="#" title="顺时针旋转"  />').click(rotateRight)[0],
						nextBtn = $('<a id="pbNextBtn" href="#" title="下一张"  />').click(next)[0]
					])[0],
	...
	*/
	...
	function setup(open) {
		...
		addEvent($(document)[0],'mousewheel',scrollZoom);
		//兼容IE和非IE浏览器原生事件绑定
	...
	function scrollZoom(e, delta) {
		...
		e=e || window.event;
		if(e.wheelDelta){
			delta=e.wheelDelta>0?1:-1;//IE/Opera/Chrome ±120
		}else if(e.detail){
			delta=e.detail>0?1:-1//Firefox ±3 
		}
