...
(function($, undefined) {
	...
	$.testRemind = (function() {
		...
		return {
			...
			hide: function() {
				//$("#" + this.id).remove();
				var obj=$("#" + this.id)
				obj.animate({opacity:0},function(){obj.remove();})
				this.display = false;
	...
	$.fn.extend({
		...
		testRemind: function(content, options) {
			...
			}).prepend($('<div></div>').css(cssOuterLimit).append(fnCreateCorner("before")).append(fnCreateCorner("after"))).fadeIn();
